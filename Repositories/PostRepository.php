<?php
class PostRepository
{

    public function getAllPost(){

        return (new Posts())->getListPost([],"ORDER BY 'name'");
    }
    public function convert_vi_to_en($str) {
        $str = preg_replace("/(à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ)/", "a", $str);
        $str = preg_replace("/(è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ)/", "e", $str);
        $str = preg_replace("/(ì|í|ị|ỉ|ĩ)/", "i", $str);
        $str = preg_replace("/(ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờ|ớ|ợ|ở|ỡ)/", "o", $str);
        $str = preg_replace("/(ù|ú|ụ|ủ|ũ|ư|ừ|ứ|ự|ử|ữ)/", "u", $str);
        $str = preg_replace("/(ỳ|ý|ỵ|ỷ|ỹ)/", "y", $str);
        $str = preg_replace("/(đ)/", "d", $str);
        $str = preg_replace("/(À|Á|Ạ|Ả|Ã|Â|Ầ|Ấ|Ậ|Ẩ|Ẫ|Ă|Ằ|Ắ|Ặ|Ẳ|Ẵ)/", "A", $str);
        $str = preg_replace("/(È|É|Ẹ|Ẻ|Ẽ|Ê|Ề|Ế|Ệ|Ể|Ễ)/", "E", $str);
        $str = preg_replace("/(Ì|Í|Ị|Ỉ|Ĩ)/", "I", $str);
        $str = preg_replace("/(Ò|Ó|Ọ|Ỏ|Õ|Ô|Ồ|Ố|Ộ|Ổ|Ỗ|Ơ|Ờ|Ớ|Ợ|Ở|Ỡ)/", "O", $str);
        $str = preg_replace("/(Ù|Ú|Ụ|Ủ|Ũ|Ư|Ừ|Ứ|Ự|Ử|Ữ)/", "U", $str);
        $str = preg_replace("/(Ỳ|Ý|Ỵ|Ỷ|Ỹ)/", "Y", $str);
        $str = preg_replace("/(Đ)/", "D", $str);
        //$str = str_replace(" ", "-", str_replace("&*#39;","",$str));
        return $str;
    }
    public function setSlug($name){
        $name = $this->convert_vi_to_en($name);
        $name = str_replace(' ','+',$name);
        $number = mt_rand(1000000,2147483647);
        $name = $name.'+'.$number;
        return $name;
    }
    public function setUrlPost($slug){
        $url = APP_DOMAIN.$slug.'html';
        return $url;
    }
    public function createPost($data){
        $slug=$this->setSlug($data['name']);

        $dataCreate = [
            'category_id'=>$data['category_id'],
            'name' => $data['name'],
            'content'=> $data['content'],
            'slug'=> $slug,
            'author_id'=> $data['author_id'],
            'publish_date'=>date("Y-m-d H:i:s"),
        ];

        return (new Posts())->create($dataCreate);
    }
    public function updatePost($data,$post){
        $slug=$this->setSlug($data['name']);
        $dataUpdate = array(
            //'category_id' => isset($data['category_id']) ? $data['category_id'] : $post->category_id,
            'name' => isset($data['name']) ? $data['name'] : $post->name,
            'content' => isset($data['content']) ? $data['content']: $post->content,
            'slug'=> $slug,
        );
        $post->update($dataUpdate);
        return $post;
    }


}


